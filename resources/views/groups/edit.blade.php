...
@section('title', 'Edit Group: ' . $group->group_name)

@section('content')
<div x-data="{
    group_name: '{{ $group->group_name }}',
    is_online: {{ $group->is_online ? 'true' : 'false' }},
    is_free: {{ $group->is_free ? 'true' : 'false' }},
    is_public: {{ ($group->is_public ?? true) ? 'true' : 'false' }},
    ...
    start_time: '{{ $group->start_time ?? '12:00' }}',
    end_time: '{{ $group->end_time ?? '14:00' }}',
    selectedStudents: {{ json_encode($group->students->pluck('student_id')) }},
    
    subcourses: {{ json_encode($subcourses) }},
    subcourseLoading: false,
    isManualName: true, // Default to true for edits
    searchTerm: '',
    courses: {{ json_encode($courses) }},
    students: {{ json_encode($students) }},
    
    init() {
        if (this.course_id && this.subcourses.length === 0) this.fetchSubcourses();
    ...
    <div class="d-flex justify-content-between align-items-center mb-5 pb-3 border-bottom theme-border">
        <div>
            <h1 class="h3 fw-bold mb-1 theme-text-main">
                <i class="fas fa-edit me-2 text-primary"></i>Edit Group: {{ $group->group_name }}
            </h1>
            <p class="text-muted small mb-0">Modify the group details and student enrollment.</p>
        </div>
        <div>
    ...
    <form action="{{ route('groups.update', $group->uuid) }}" method="POST">
        ...
        <div class="row g-4">
                        ...
                        <div class="col-md-6 mt-4">
                            ...
                            <select name="subcourse_id" class="form-select theme-card border theme-border theme-text-main" x-model="subcourse_id" :disabled="subcourseLoading || !course_id">
                                ...
                                <template x-for="sub in subcourses" :key="sub.subcourse_id">
                                    <option :value="sub.subcourse_id" x-text="sub.subcourse_name || `Part ${sub.subcourse_number}`" :selected="sub.subcourse_id == subcourse_id"></option>
                                </template>
                            </select>
                        </div>
                        <div class="col-md-12">
            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 theme-card h-100 overflow-hidden d-flex flex-column">
                    ...
                    <div class="card-body p-4 d-flex flex-column">
                        ...
                        <div class="flex-grow-1 overflow-auto pe-2" style="max-height: 500px;">
                            <template x-for="student in filteredStudents" :key="student.student_id">
                                <div 
                                    ...
                                    :class="selectedStudents.includes(student.student_id) ? 'bg-primary bg-opacity-10 border-primary' : 'theme-card'"
                                ...
                                >
                                    ...
                                    <div class="ms-2 flex-grow-1">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mt-1">
                                            <span class="badge rounded-pill smaller px-2 py-1"
                                                :class="{
                                                    'bg-success bg-opacity-10 text-success': student.payment_status === 'paid',
                                                    'bg-warning bg-opacity-10 text-warning': student.payment_status === 'partial',
                                                    'bg-danger bg-opacity-10 text-danger': student.payment_status === 'unpaid',
                                                    'bg-info bg-opacity-10 text-info': student.payment_status === 'new_student'
                                                }"
                                                x-text="student.payment_status.replace('_', ' ').toUpperCase()"
                                            ></span>
                                            <span x-show="student.balance > 0" class="text-danger smaller fw-bold" x-text="'-' + student.balance + ' EGP'"></span>
                                        </div>
                                    </div>
                                </div>
                            </template>
                        ...
                        <div class="mt-4 pt-3 border-top theme-border">
                            ...
                            <button type="submit" class="btn btn-primary w-100 rounded-pill py-2 shadow">
                                Update Group
                            </button>
                        </div>
                    </div>
