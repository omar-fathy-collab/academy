...
@section('content')
    @php
        $teacherData = [
            'teacher_name' => old('teacher_name', $teacher->teacher_name),
            'email' => old('email', $teacher->email),
            'hire_date' => old('hire_date', $teacher->hire_date),
        ];
    @endphp
    <script id="teacher-data" type="application/json">
        @json($teacherData)
    </script>
    <div class="container-fluid py-4 min-vh-100" x-data="Object.assign(JSON.parse(document.getElementById('teacher-data').textContent), {})">
        <div class="d-flex justify-content-between align-items-center mb-5 pb-3 border-bottom theme-border">
            <div>
                <h1 class="h3 fw-bold mb-1 text-primary">Edit Teacher Profile</h1>
                <p class="text-muted small mb-0">Update information for {{ $teacher->teacher_name }}</p>
            </div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <form action="{{ route('teachers.update', $teacher->teacher_id) }}" method="POST" class="card border-0 shadow-sm rounded-4 overflow-hidden theme-card">
                    @csrf
                    @method('PUT')
                    <div class="card-header theme-card-header p-4 border-bottom-0">
                        <h5 class="fw-bold mb-0 theme-text-main">Teacher Information</h5>
                    </div>
            ...
            <div class="col-lg-4 mt-4 mt-lg-0">
                <div class="card border-0 shadow-sm rounded-4 theme-card p-4 sticky-top" style="top: 2rem;">
                    <div class="text-center mb-4">
                        ...
                        <h5 class="fw-bold mb-1 theme-text-main" x-text="teacher_name"></h5>
                        <p class="text-muted small">ID: {{ $teacher->teacher_id }}</p>
                    </div>

                    <hr class="theme-border opacity-25" />

                    <div class="info-list small">
                        <div class="d-flex justify-content-between mb-2">
                            <span class="text-muted">Status:</span>
                            <span class="badge {{ $teacher->is_active ? 'bg-success' : 'bg-danger' }} rounded-pill shadow-sm">
