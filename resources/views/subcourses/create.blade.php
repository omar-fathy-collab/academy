        ...
        <div class="col-lg-8 col-xl-7">
            <div class="d-flex align-items-center mb-4 gap-3">
                ...
                <h4 class="fw-bold mb-0 theme-text-main">
                    <i class="fas fa-layer-group text-warning me-2"></i>Add Subcourse
                </h4>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 theme-card overflow-hidden">
                <div class="card-body p-4">
                    <form action="{{ route('subcourses.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            ...
                            <select name="course_id" class="form-select rounded-3 theme-badge-bg theme-text-main theme-border px-3 py-2 @error('course_id') is-invalid @enderror" required>
                                ...
                                @foreach($courses as $c)
                                    <option value="{{ $c->course_id }}" {{ (isset($selected_course_id) && $selected_course_id == $c->course_id) || old('course_id') == $c->course_id ? 'selected' : '' }}>
                                        {{ $c->course_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row g-4">
                            <div class="col-md-8">
                                ...
                                <input type="text" name="subcourse_name" value="{{ old('subcourse_name') }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="subcourse_number" value="{{ old('subcourse_number') }}"
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" class="form-control rounded-3 theme-badge-bg theme-text-main theme-border px-3 py-2" rows="3">{{ old('description') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end mt-5 pt-3 border-top theme-border">
                            <button type="submit" class="btn btn-warning rounded-pill px-5 py-2 fw-bold shadow-sm">
                                <i class="fas fa-save me-2"></i>Add Subcourse
                            </button>
                        </div>
                    </form>
                </div>
