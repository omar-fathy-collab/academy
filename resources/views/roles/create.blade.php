...
<div class="container-fluid py-4 theme-text-main" x-data="roleForm({
    ...
    <div class="d-flex align-items-center mb-4">
        ...
        <div>
            <h2 class="fw-bold theme-text-main mb-1">Create New Role</h2>
            <p class="text-muted small mb-0">Define specific access rules and permissions</p>
        </div>
    </div>
        ...
        <div class="row g-4">
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 theme-card p-4 h-100">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" class="form-control rounded-3 py-2 theme-badge-bg theme-text-main theme-border shadow-none" placeholder="e.g., Academic Auditor" required value="{{ old('name') }}" />
                        @error('name') <div class="text-danger smaller mt-1">{{ $message }}</div> @enderror
                        <p class="smaller text-muted mt-2">The name must be unique and descriptive of the access level.</p>
                    </div>
                    ...
                    <div class="alert alert-info border-0 rounded-4 p-3 mb-0 smaller shadow-sm">
                        ...
                        <p class="mb-2">Total permissions selected:</p>
                                    ...
                                    <div class="p-3 rounded-4 border theme-border theme-badge-bg h-100">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <div class="form-check form-switch mb-0">
                                                <input class="form-check-input" type="checkbox" @change="toggleGroup(groupName, $event.target.checked)" :checked="isGroupSelected(groupName)">
                                            </div>
                                        </div>
            ...
            <div class="col-12 mt-4 text-end">
                <button type="submit" class="btn btn-primary fw-bold rounded-pill px-5 shadow-sm transition-hover py-3">
                    <i class="fas fa-save me-2"></i> Save & Assign Role
                </button>
            </div>
        </div>
    </form>
...
function roleForm(config) {
    return {
        ...
        isGroupSelected(groupName) {
            const groupPermNames = this.permissions[groupName].map(p => p.name);
            return groupPermNames.every(name => this.selectedPermissions.includes(name));
        }
    };
}
</script>
