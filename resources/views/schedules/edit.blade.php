...
<div x-data="scheduleForm({
    availableGroups: {{ $groups->map(fn($g) => ['id' => $g->group_id, 'name' => $g->group_name, 'start' => $g->start_date, 'end' => $g->end_date, 'course' => $g->course->course_name])->toJson() }},
    checkUrl: '{{ route('schedules.check-availability', ['exclude_schedule_id' => $schedule->schedule_id]) }}',
    initialData: {
        ...
        end_date: '{{ $schedule->end_date ?? $defaultEndDate }}'
    }
})">
    <!-- Page Header -->
    <div class="d-flex justify-content-between align-items-center mb-5 pb-3 border-bottom theme-border">
        <div>
            ...
            <p class="text-muted small mb-0">Modify an existing recurring class timetable for an academy group</p>
        </div>
                ...
                <div class="card-body p-4 p-md-5">
                    <form action="{{ route('schedules.update', $schedule->schedule_id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="group_id" class="form-select theme-card theme-border theme-text-main py-2" x-model="formData.group_id" @change="onGroupChange()" required>
                                <template x-for="group in availableGroups">
                                    <option :value="group.id" :selected="group.id == formData.group_id" x-text="group.name"></option>
                                </template>
                            </select>
                        ...
                        <div x-show="checkingAvailability" class="alert alert-info py-2 rounded-3 mb-4 d-flex align-items-center">
                            <i class="fas fa-spinner fa-spin me-2"></i> Re-validating room occupancy...
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-warning rounded-pill py-3 fw-bold shadow" :disabled="!isAvailable || checkingAvailability">
                                <i class="fas fa-save me-2"></i> Update Schedule
                            </button>
                            <a href="{{ route('schedules.index') }}" class="btn btn-link text-muted">Cancel Changes</a>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 theme-card h-100">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-4 p-3 bg-light rounded-3 theme-border border">
                        <p class="small text-muted mb-2">Current Duration</p>
                        <h5 class="mb-0 text-primary fw-bold" x-text="calculateDuration()">--</h5>
                    </div>
                    <div class="alert alert-info small border-0 py-2">
                        <i class="fas fa-info-circle me-1"></i> Modifying this schedule will update it for all future class sessions.
                    </div>
                </div>
            </div>
        </div>
...
function scheduleForm(config) {
    return {
        ...
        isAvailable: true, // Start as available since we're editing an existing one
        availabilityMessage: '',
        selectedGroup: null,
